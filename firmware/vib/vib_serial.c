...
static const ShellConfig uart_shell_cfg = {
...
void vib_Serial_Loop(void){
    if(!uart_shelltp){
        uart_shelltp = shellCreate(&uart_shell_cfg, SHELL_WA_SIZE, NORMALPRIO);
    }
    else if(chThdTerminatedX(uart_shelltp)){
        chThdRelease(uart_shelltp);
        uart_shelltp = NULL;
    }
}
